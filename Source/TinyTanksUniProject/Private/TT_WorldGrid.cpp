...
ATT_GridCell* ATT_WorldGrid::GetRandomEmptyCell()
{
	...
	if (cellArray.Num() > 0)
	{
		for (int i = 0; i < cellArray.Num(); i++)
		{
			if (cellArray[i] && cellArray[i]->itemToSpawn == E_ItemToSpawn::ITS_none && cellArray[i] && cellArray[i]->itemToSpawn == E_ItemToSpawn::ITS_filled && cellArray[i]->floorItemToSpawn != E_FloorItemToSpawn::FITS_destroyed && cellArray[i]->floorItemToSpawn != E_FloorItemToSpawn::FITS_none)
			{
				arrayOfEmpty.Add(cellArray[i]);
			}
		}
