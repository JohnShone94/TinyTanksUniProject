...
ATT_Boost::ATT_Boost()
{
	...
	boostStaticMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("BoostPU"));
	RootComponent = boostStaticMesh;

	boostCollision = CreateDefaultSubobject<UBoxComponent>(TEXT("Pickup"));
...
void ATT_Boost::OnOverlapBegin(class UPrimitiveComponent* OverlappedComp, class AActor* OtherActor, class UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult & SweepResult)
{
	if ((OtherActor != nullptr) && (OtherActor != this) && (OtherComp != nullptr))
	{
		...
		UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), boostParticleSystem, GetActorLocation());
